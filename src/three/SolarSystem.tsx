...
export default function SolarSystem({ 
  ...
  // Sin filtrar: todos los asteroides (optimizado con Points)
  ...
  return (
    ...
    >
      {/* Ensure the renderer clears to black to avoid any gray CSS background */}
      {/* @ts-ignore */}
      <color attach="background" args={[0x000000]} />
      ...
      />
  {/* Fondo de Vía Láctea (desactivable con ?nobg) */}
  {!disableBG && <SpaceBackground />}
      
      {/* Iluminación fija a 300x (normalizada con base 170x) */}
      {(() => {
        const norm = 300 / 170; // ~1.7647
        return (
          <>
            <ambientLight intensity={0.18 * norm} color="#aab" />
            <hemisphereLight args={["#e0f0ff", "#222222", 0.25 * norm]} />
            <pointLight position={[0, 0, 0]} intensity={1200 * norm} distance={5000} decay={2} color="#ffd27a" />
            <pointLight position={[600, 400, 600]} intensity={0.25 * norm} distance={0} decay={2} color="#cde" />
          </>
        );
      })()}
    ...
    {/* HDRI environment removed for stability */}
      ...
      <Suspense fallback={(
        ...
        <Planet
          ...
          isSun
        />
        {/* Halo del Sol: único y brillante */}
        <mesh position={[0,0,0]}>
          <sphereGeometry args={[10, 32, 32]} />
          <meshBasicMaterial color="#ffcc44" depthWrite={false} />
        </mesh>
      </Suspense>
