...
export default function EyesOnAsteroids() {
  ...
  // Estadísticas siguen mostrando totales de BD
  const stats = useMemo(() => {
    ...
    <div className="eoa-root">
      ...
      {!showLeftPanel && (
        <button
          ...
          aria-label="Mostrar Estadísticas y Controles"
        >
          Mostrar Panel
        </button>
      )}
      {!showTimeControls && (
        <button
          ...
          aria-label="Mostrar Controles de Tiempo"
        >
          Mostrar Tiempo
        </button>
      )}
      {/* Simulación 3D (se renderiza detrás para precargar mientras está el loader) */}
      <SolarSystem
        neos={filteredNeos}
  ...
  {/* Información del asteroide seleccionado movida junto al NEO (renderizada dentro de SolarSystem con <Html />) */}
      ...
      <div className={`time-controls time-controls--compact ${showTimeControls ? 'is-visible' : 'is-hidden'}`}>
        <div className="tc-badge">
          <div>Fecha: {displayDate.toLocaleDateString('es-ES', { year: 'numeric', month: '2-digit', day: '2-digit' })}</div>
          <div>Hora: {displayDate.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</div>
          <div>JD: {displayJD.toFixed(5)}</div>
        </div>
        <div className="tc-grid">
          {/* Columna izquierda: Velocidad + / etiqueta / Velocidad - */}
          <div className="tc-col tc-col--left">
            <button className="btn" onClick={() => setSpeedIndex(i => Math.min(speedPresets.length - 1, i + 1))}>Velocidad +</button>
            <div className="speed-label">Velocidad {currentSpeed.label}</div>
            <button className="btn" onClick={() => setSpeedIndex(i => Math.max(0, i - 1))}>Velocidad -</button>
          </div>
          ...
          <div className="tc-col tc-col--center">
            <button className="btn btn--danger-outline" onClick={() => setTimeState(s => ({ ...s, isPlaying: !s.isPlaying }))}>
              {timeState.isPlaying ? 'Pausar' : 'Reproducir'}
            </button>
            ...
            <button className="panel-close" onClick={() => setShowTimeControls(false)} aria-label="Ocultar controles de tiempo">Ocultar</button>
          </div>

          {/* Columna derecha: Futuro ▶ / Dirección / Pasado ◀ */}
          <div className="tc-col tc-col--right">
            <button className="btn" onClick={() => { setDirection(1); setTimeState(s => ({ ...s, isPlaying: true })); }}>Futuro ▶</button>
            <div className="dir-label">Dirección: {direction === 1 ? 'Futuro' : 'Pasado'}</div>
            <button className="btn" onClick={() => { setDirection(-1); setTimeState(s => ({ ...s, isPlaying: true })); }}>Pasado ◀</button>
          </div>
        </div>
      </div>
    </div>
